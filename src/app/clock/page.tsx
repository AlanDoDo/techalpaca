...
export default function DailyEnglishPage() {
	...
	useEffect(() => {
		...
		setActiveEntryId(prev => {
			...
			const allEntries = [...history, ...confirmedEntries]
	...
	const libraryTitle = confirmedEntries.length > 0 ? '已确认句库' : '候选句子'
	const libraryDescription = confirmedEntries.length > 0
		? '这里的句子已经被你确认收录到仓库，会被遗忘曲线优先调用做后续复习。'
		: '这里先展示接口返回的候选句子。点击上方确认后，它们会被写入仓库并进入你的复习句库。'
	...
	const confirmEntry = async (entry: DailyEnglishEntry) => {
		...
		try {
			...
			if (result.alreadyExists) {
				toast.info('这条句子已经在你的仓库句库里了')
			} else {
				toast.success('已确认收录，后续会进入遗忘曲线复习')
			}
		} catch (error: any) {
			console.error('Failed to confirm daily English:', error)
			toast.error(`确认失败: ${error?.message || '发生了未知错误'}`)
		} finally {
	...
	const handleChoosePrivateKey = async (file: File) => {
		try {
			...
			toast.error('私钥读取失败，请重新选择 .pem 文件')
			setPendingConfirmEntry(null)
		}
	}
	...
	if (isLoading && !data) {
		return (
			<div className='mx-auto flex min-h-screen w-full max-w-6xl flex-col gap-6 px-4 pt-28 pb-16 sm:px-6'>
	...
	return (
		<>
			...
			<div className='relative overflow-hidden px-4 pt-28 pb-14 sm:px-6'>
				<div className='pointer-events-none absolute inset-0 overflow-hidden'>
					<div className='absolute top-[-120px] left-[12%] h-72 w-72 rounded-full bg-[rgba(217,109,37,0.12)] blur-3xl' />
					<div className='absolute top-[22%] right-[-80px] h-80 w-80 rounded-full bg-[rgba(53,191,171,0.12)] blur-3xl' />
				...
				<div className='relative mx-auto max-w-6xl space-y-8'>
					<motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className='flex flex-wrap items-center justify-between gap-3'>
						<Link href='/' className='inline-flex items-center gap-2 rounded-full border border-white/80 bg-white/75 px-3 py-1.5 text-xs font-medium text-[#75553a] shadow-[0_12px_32px_-22px_rgba(70,36,17,0.8)] transition hover:border-[#d99a6a]/40 hover:text-[#c16424]'>
							<ArrowLeft className='h-3.5 w-3.5' />
							返回首页
						</Link>
						<div className='rounded-full border border-white/80 bg-white/70 px-3 py-1.5 text-xs font-medium text-[#886a56]'>
							{formatDateLabel()} · 每天一句英语口语
						</div>
					</motion.div>
					...
					<div className='grid gap-6 xl:grid-cols-[minmax(0,1.15fr)_360px]'>
						<motion.section
							...
							className='relative overflow-hidden rounded-[36px] border border-white/80 bg-[linear-gradient(135deg,rgba(255,250,244,0.94),rgba(255,240,228,0.98)_55%,rgba(249,232,218,0.96))] p-6 shadow-[0_40px_120px_-65px_rgba(88,41,17,0.45)] sm:p-8'>
							<div className='absolute top-0 right-0 h-40 w-40 translate-x-10 -translate-y-8 rounded-full bg-[rgba(217,109,37,0.14)] blur-3xl' />
							...
							<div className='relative flex flex-col gap-8'>
								<div className='flex flex-wrap items-start justify-between gap-4'>
									<div className='space-y-4'>
										<div className='inline-flex items-center gap-2 rounded-full border border-white/80 bg-white/75 px-3 py-1 text-[11px] font-semibold tracking-[0.28em] text-[#9a5b2c] uppercase'>
											<Sparkles className='h-3.5 w-3.5' />
											Daily English
										</div>
										<div>
											<div className='text-[12px] font-medium tracking-[0.22em] text-[#94735c] uppercase'>
												Today&apos;s sentence #{String(Math.max(todayIndex, 1)).padStart(2, '0')}
											</div>
											<h1 className='mt-3 max-w-3xl text-3xl leading-tight font-semibold text-[#4c311f] sm:text-5xl'>{activeEntry.english}</h1>
									...
									<div className='flex flex-wrap items-center justify-end gap-3'>
										...
										<button
											...
											{currentIsConfirmed ? '已确认收录' : isConfirming ? '确认中...' : isAuth ? '确认并收录' : '导入私钥后确认'}
										</button>
									</div>
								</div>
								...
								<div className='rounded-[28px] border border-white/85 bg-white/70 px-5 py-4 text-sm leading-7 text-[#735746] shadow-[0_30px_60px_-45px_rgba(0,0,0,0.35)]'>
									确认后，这条句子会被写入仓库文件 `src/config/daily-english-confirmed.json`，并进入遗忘曲线复习。后续页面和首页都会优先读取你的已确认句库。
								</div>
								...
								<div className='grid gap-4 md:grid-cols-[1.05fr_0.95fr]'>
									<div className='rounded-[28px] border border-white/85 bg-white/72 p-5 shadow-[0_30px_60px_-45px_rgba(0,0,0,0.4)]'>
										...
										<div className='mt-4 text-sm leading-7 text-[#705748]'>
											<p><span className='font-semibold text-[#4c311f]'>发音提示：</span>{activeEntry.pronunciation}</p>
											<p className='mt-3'><span className='font-semibold text-[#4c311f]'>适用场景：</span>{activeEntry.scenario}</p>
									...
									<div className='rounded-[28px] border border-white/85 bg-[#fffdf8]/85 p-5 shadow-[0_30px_60px_-45px_rgba(0,0,0,0.4)]'>
										...
										<div className='mt-5 rounded-2xl bg-[linear-gradient(135deg,rgba(53,191,171,0.10),rgba(217,109,37,0.08))] px-4 py-3 text-xs leading-6 text-[#6a5a4b]'>
											现在内容默认来自 Tatoeba 官方 API；只有你点击确认的句子，才会进入仓库句库并参与后续复习调度。
										</div>
									</div>
								</div>
							</div>
						...
						<motion.aside initial={{ opacity: 0, y: 28 }} animate={{ opacity: 1, y: 0 }} className='space-y-4'>
							<div className='rounded-[30px] border border-white/80 bg-white/72 p-5 shadow-[0_35px_80px_-52px_rgba(0,0,0,0.45)]'>
								<div className='text-[11px] font-semibold tracking-[0.24em] text-[#94735c] uppercase'>学习面板</div>
								...
								<div className='mt-5 rounded-[24px] border border-dashed border-[#ead8cb] bg-[#fffaf5] p-4'>
									...
									<p className='mt-3 text-xs leading-6 text-[#8a7463]'>今天确认收录的句子会自动进入下一次复习安排；如果觉得模糊，点“再看看”会把它拉回更近的一天。</p>
								</div>
							</div>
							...
							<div className='rounded-[30px] border border-white/80 bg-[linear-gradient(180deg,rgba(255,255,255,0.8),rgba(255,247,240,0.95))] p-5 shadow-[0_35px_80px_-52px_rgba(0,0,0,0.45)]'>
								<div className='text-[11px] font-semibold tracking-[0.24em] text-[#94735c] uppercase'>当前状态</div>
								<div className='mt-4 space-y-3 text-sm leading-7 text-[#6f5646]'>
									<div className='rounded-2xl bg-white/85 px-4 py-3'>
										<div className='text-xs text-[#9a806e]'>学习状态</div>
										<div className='mt-1 font-semibold text-[#4c311f]'>{getDailyEnglishReviewLabel(activeProgress)}</div>
									</div>
									<div className='rounded-2xl bg-white/85 px-4 py-3'>
										<div className='text-xs text-[#9a806e]'>下次复习</div>
										<div className='mt-1 font-semibold text-[#4c311f]'>{formatReviewDate(activeProgress?.nextReviewOn)}</div>
									</div>
									<div className='rounded-2xl bg-white/85 px-4 py-3'>
										<div className='text-xs text-[#9a806e]'>仓库句库</div>
										<div className='mt-1 font-semibold text-[#4c311f]'>{confirmedEntries.length} 条已确认句子</div>
									</div>
									<div className='rounded-2xl bg-white/85 px-4 py-3'>
										<div className='text-xs text-[#9a806e]'>数据来源</div>
										<a href={data.source.url} target='_blank' rel='noreferrer' className='mt-1 inline-flex text-sm font-semibold text-[#4c311f] underline decoration-[#dba073] underline-offset-4'>
											{data.source.label}
										</a>
									</div>
								</div>
					...
					<section className='grid gap-6 lg:grid-cols-[minmax(0,0.95fr)_minmax(0,1.05fr)]'>
						<motion.div initial={{ opacity: 0, y: 26 }} animate={{ opacity: 1, y: 0 }} className='rounded-[32px] border border-white/80 bg-white/72 p-5 shadow-[0_35px_80px_-55px_rgba(0,0,0,0.45)] sm:p-6'>
							...
							<p className='mt-2 text-sm leading-7 text-[#866d5d]'>
								{confirmedEntries.length > 0
									? '现在的复习队列优先来自你已经确认收录到仓库的句子。点卡片会切换到上面的详细讲解区。'
									: '先从你已经看过的候选句子里做节奏化复习；确认收录后，这里会优先切到仓库句库。'}
							</p>

							<div className='mt-5 space-y-3'>
								{reviewQueue.length === 0 ? (
									<div className='rounded-[26px] border border-dashed border-[#e8d7ca] bg-[#fffaf5] px-5 py-6 text-sm leading-7 text-[#866d5d]'>
										今天的复习已经清空了。继续确认新句子，或者明天回来时，系统会按遗忘曲线自动把旧句子推回来。
									</div>
								) : (
											...
											<div key={entry.id} className='rounded-[26px] border border-white/85 bg-[#fffdf9] p-4 shadow-[0_22px_40px_-35px_rgba(0,0,0,0.4)]'>
												<button type='button' onClick={() => setActiveEntryId(entry.id)} className='w-full text-left'>
													<div className='flex items-start justify-between gap-4'>
														<div>
															<div className='text-base font-semibold text-[#4c311f]'>{entry.english}</div>
														...
														<span className='shrink-0 rounded-full bg-[#fff2e5] px-3 py-1 text-[11px] font-medium text-[#b76a2e]'>
															{entryProgress?.nextReviewOn === toDateKey() ? '今天' : '补复习'}
														</span>
													</div>
												</button>
												<div className='mt-4 flex flex-wrap gap-2'>
						...
						<motion.div initial={{ opacity: 0, y: 32 }} animate={{ opacity: 1, y: 0 }} className='rounded-[32px] border border-white/80 bg-[linear-gradient(180deg,rgba(255,255,255,0.8),rgba(251,245,238,0.95))] p-5 shadow-[0_35px_80px_-55px_rgba(0,0,0,0.45)] sm:p-6'>
							<div className='flex items-center gap-2 text-sm font-semibold text-[#6d4a30]'>
									...
									return (
										<button
											...
											<div className='flex items-start justify-between gap-3'>
												<span className='rounded-full bg-[#f7eee6] px-2.5 py-1 text-[10px] font-semibold tracking-[0.18em] text-[#9e6e47] uppercase'>
													#{String(index + 1).padStart(2, '0')}
												</span>
												<span className='text-[11px] font-medium text-[#9a7f6c]'>{getDailyEnglishReviewLabel(record)}</span>
											</div>
											<div className='mt-3 line-clamp-3 text-sm leading-6 font-medium text-[#4f3522]'>{entry.english}</div>
...
function StatCard({ label, value, highlight = false }: { label: string; value: number; highlight?: boolean }) {
	return (
		<div className={cn('rounded-[22px] px-4 py-4', highlight ? 'bg-[#fff2e5]' : 'bg-[#fffaf5]')}>
			<div className='text-[11px] font-medium tracking-[0.18em] text-[#977a66] uppercase'>{label}</div>
